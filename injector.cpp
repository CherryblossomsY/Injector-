...
{
	PROCESSENTRY32 ProcessInformationStructure;
	ProcessInformationStructure.dwSize = sizeof(ProcessInformationStructure);
	HANDLE ProcessSnapshot = CreateToolhelp32Snapshot(15, 0);
	USES_CONVERSION;
	Process32First(ProcessSnapshot, &ProcessInformationStructure);
	do {
		if (strcmp(W2A(ProcessInformationStructure.szExeFile), ProcessName) == 0)
		{
			CloseHandle(ProcessSnapshot);
			return ProcessInformationStructure.th32ProcessID;
		}
	} while (Process32Next(ProcessSnapshot, &ProcessInformationStructure));
	CloseHandle(ProcessSnapshot);
	return NULL;
}

bool ManualMapDll(const char* DesProcName, const char* DllName) {
	...
	GetFileAttributesA(DllName);

	std::ifstream File(DllName, std::ios::binary | std::ios::ate);
	FileSize = File.tellg();
